<script lang="ts">
  import { getApiBaseUrl, getJourneyTrace, startJourney, submitJourneyResult } from '$lib/api';
  ...
  function rememberInteraction(response: JourneyRunResponse): void {
    ...
    history = [...history, { sequence: history.length + 1, interactionId, status: response.status }];
  }
...
<svelte:head>
  ...
  <meta name="description" content="A developer-facing prototype of a deterministic, server-driven service journey" />
</svelte:head>
...
<div class="phase"><div><strong>Research prototype</strong><span>This is not a live government service.</span></div></div>

<main>
  <div class="intro">
    <span class="eyebrow">Comparable journey execution spike</span>
    <h1>Change your driving-licence address</h1>
    <p>This demonstration shows a deterministic executor following a journey controlled by the service API. The developer panel exposes the state and raw transport trace.</p>
  </div>

  {#if errorMessage}<div class="error-banner" role="alert"><strong>There is a problem</strong><span>{errorMessage}</span></div>{/if}
    ...
    <section class="service-card" aria-live="polite">
      {#if !run}
        <div class="card-body">
          ...
          <p>The browser asks the Python executor to start the journey. The service returns the first interaction and controls every subsequent transition.</p>
          <ul><li>Choose postcode lookup or manual entry</li><li>Enter and review an address</li><li>Confirm before completion</li></ul>
          <button class="primary" type="button" onclick={start} disabled={starting}>{starting ? 'Starting journey…' : 'Start journey'}</button>
        </div>
      {:else if run.terminal}
        <div class="card-body completion">
          ...
          <p>The service returned a terminal status. No further action was selected by the executor or browser.</p>
          <dl><div><dt>Final status</dt><dd>{run.status}</dd></div><div><dt>Interactions</dt><dd>{history.length}</dd></div><div><dt>Trace events</dt><dd>{traceEvents.length}</dd></div></dl>
          <button class="secondary" type="button" onclick={reset}>Start another run</button>
        </div>
      {:else if interaction}
        <div class="card-body">
          <div class="interaction-meta"><span class="step-label">Interaction {history.length}</span><code>{interaction.id ?? 'unnamed_interaction'}</code></div>
          <h2>{title}</h2>
          {#if description}<p>{description}</p>{/if}
          <DataSummary data={summaryData} />
          {#key interactionKey}<SchemaForm {interaction} disabled={submitting} onSubmit={submit} />{/key}
        </div>
      {/if}
    </section>

    <DeveloperPanel {run} {traceEvents} {history} apiBaseUrl={API_BASE_URL} refreshing={refreshingTrace} onRefresh={refreshTrace} />
  </div>
</main>

<footer><div><strong>Agentic Legibility</strong><span>Prototype for exploring server-driven journeys and comparable traces.</span></div></footer>
...
<style>
  ...
  .intro { max-width: 49rem; margin-bottom: 2rem; }
  ...
  .completion dt { font-weight: 700; } .completion dd { margin: 0; }
