...
export async function runComparison(form: GovForm, messages: ChatMessage[]) {
  ...
  // A failure here should not blank the page — fall back to a spoken error.
  let chat;
  try {
    ...
    chat = {
      ...
      rationale: message,
      telemetry: { model: "unknown", latencyMs: 0, inputTokens: 0, outputTokens: 0 },
    };
  }
  ...
  return {
    ...
    // Raw per-turn cost of the agent call, surfaced for the run log.
