...
`checkpoint.id` is required. `checkpoint_runner.py` resolves the ID to `<id>.json` in this directory and starts each evaluation repetition from a fresh `InterpreterState` built from the captured stack frames, variables, transcript, step counter and environment. `invoker_state_id` values are rehydrated from the workflow definition so nested subprocesses return to their parent frames normally.
